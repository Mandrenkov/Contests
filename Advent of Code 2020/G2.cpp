...
void solution() {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for (int i = 4; i < tokens.size(); i += 4) {
            const int quantity = stoi(tokens[i]);
            const auto insider = tokens[i + 1] + " " + tokens[i + 2];
            bags[outsider].emplace_back(quantity, insider);
        }
    }
    const function<int(const string&)> explore = [&](const string& outsider) {
        int counter = 1;
        for (const auto& [quantity, insider] : bags[outsider]) {
            counter += quantity * explore(insider);
        }
        return counter;
    };
    int insiders = explore("shiny gold") - 1;
    cout << insiders << '\n';
}
