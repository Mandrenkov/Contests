...
constexpr int NUM_MOVES = 10000000;
...
void solution() {
    ...
    for (char input : inputs) {
        int value = input - '0';
        Cup* cup = new Cup{value, prev, nullptr};
        if (prev != nullptr) {
            prev->next = cup;
        }
        if (head == nullptr) {
    ...
    for (int value = inputs.size() + 1; value <= NUM_CUPS; ++value) {
        Cup* cup = new Cup{value, prev, nullptr};
        prev->next = cup;
        prev = cup;
    }
    ...
    long long product = static_cast<long long>(seek(head, 1)->value) *
                        static_cast<long long>(seek(head, 2)->value);
    cout << product << '\n';
}
