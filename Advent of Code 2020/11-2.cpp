...
void solution() {
    ...
    const auto simulate = [&](const Seats& seats, int r, int c) {
        int counter = 0;
        for (int dr = -1; dr <= 1; ++dr) {
            for (int dc = -1; dc <= 1; ++dc) {
                ...
                int row = r + dr;
                int col = c + dc;
                while (0 <= row && row < N && 0 <= col && col < M && seats[row][col] == '.') {
                    row += dr;
                    col += dc;
                }
                if (0 <= row && row < N && 0 <= col && col < M && seats[row][col] == '#') {
                    ++counter;
                }
            }
        }
    ...
    do {
        ...
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < M; ++j) {
                ...
                int adjacent = simulate(prev, i, j);
                if (adjacent == 0) {
                    next[i][j] = '#';
                } else if (adjacent >= 5) {
                    next[i][j] = 'L';
