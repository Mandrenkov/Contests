...
constexpr long long MOD = 20201227;

void solution() {
    long long public_keys[2];
    cin >> public_keys[0] >> public_keys[1];
    ...
    long long shared_key = 1;
    for (int i = 0; i < secret_key; ++i) {
        shared_key = public_keys[1] * shared_key % MOD;
    }
    cout << shared_key << '\n';
}
