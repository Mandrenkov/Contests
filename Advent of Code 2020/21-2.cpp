...
void solution() {
    ...
    while (dangerous.size() != candidate_map.size()) {
        pair<string, string> candidate;
        for (const auto& [allergen, candidates] : candidate_map) {
            if (candidates.size() == 1) {
                candidate = {allergen, *candidates.begin()};
                break;
            }
        }
        for (auto& [_, candidates] : candidate_map) {
            candidates.erase(candidate.second);
        }
        dangerous.emplace(candidate);
    }

    for (auto it = dangerous.begin(); it != dangerous.end(); ++it) {
        cout << it->second;
        if (next(it) != dangerous.end()) {
            cout << ',';
        }
    }
    cout << '\n';
