...
void solution() {
    ...
    for (int i = 0; i < N; ++i) {
        string range;
        cin >> range;
        int lo = stoi(range.substr(0, range.find_first_of('-')));
        int hi = stoi(range.substr(range.find_first_of('-') + 1));
        ...
        int frequency = count(password.begin(), password.end(), letter);
        valid += lo <= frequency && frequency <= hi;
    }
    cout << valid << '\n';
}
