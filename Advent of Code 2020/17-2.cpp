...
ostream& operator<<(ostream& out, const tuple<T, U, V, W>& triple) {
    return out << '(' << get<0>(triple) << ", " << get<1>(triple) << ", " << get<2>(triple) << ", " << get<3>(triple) << ')';
}
...
// Hash Template Specializations
template<class T, class U, class V, class W>
struct hash<std::tuple<T, U, V, W>> {
    size_t operator()(const std::tuple<T, U, V, W>& triple) const noexcept {
        ...
        const auto h4 = std::hash<W>{}(std::get<3>(triple));
...
void solution() {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for (int j = 0; j < row.size(); ++j) {
            if (row[j] == '#') {
                prev.emplace(j, i, 0, 0);
            }
        }
    }
    for (int i = 0; i < 6; ++i) {
        unordered_set<Cube> candidates;
        for (const auto [x, y, z, w] : prev) {
            for (int dx = -1; dx <= 1; ++dx) {
                for (int dy = -1; dy <= 1; ++dy) {
                    for (int dz = -1; dz <= 1; ++dz) {
                        for (int dw = -1; dw <= 1; ++dw) {
                            candidates.emplace(x + dx, y + dy, z + dz, w + dw);
                        }
                    }
                }
            }
        ...
        for (const auto& candidate : candidates) {
            const auto [x, y, z, w] = candidate;
            int neighbours = 0;
            for (int dx = -1; dx <= 1; ++dx) {
                for (int dy = -1; dy <= 1; ++dy) {
                    for (int dz = -1; dz <= 1; ++dz) {
                        for (int dw = -1; dw <= 1; ++dw) {
                            Cube cube = {x + dx, y + dy, z + dz, w + dw};
