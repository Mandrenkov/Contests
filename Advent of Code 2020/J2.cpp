...
void solution() {
    int N;
    cin >> N;
    // The joltage at index |N| is the charging outlet joltage (i.e., 0).
    vector<int> joltages(N + 1, 0);
    for (int i = 0; i < N; ++i) {
        cin >> joltages[i];
    }
    sort(joltages.begin(), joltages.end());
    // The device joltage is 3 more than the largest adapter joltage.
    int device = joltages.back() + 3;
    joltages.emplace_back(device);
    // Counting the number of ways to reach |device| jolts is similar to the change-making problem.
    vector<long long> dp(device + 1, 0);
    dp[0] = 1;
    for (int i = 1; i < joltages.size(); ++i) {
        int jolts = joltages[i];
        dp[jolts] += (jolts - 1 >= 0) ? dp[jolts - 1] : 0;
        dp[jolts] += (jolts - 2 >= 0) ? dp[jolts - 2] : 0;
        dp[jolts] += (jolts - 3 >= 0) ? dp[jolts - 3] : 0;
    }
    cout << dp.back() << '\n';
}
