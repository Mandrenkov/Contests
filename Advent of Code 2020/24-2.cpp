...
void solution() {
    unordered_set<pair<int, int>> prev_flipped;
    string line;
    while (getline(cin, line) && !line.empty()) {
        ...
        if (prev_flipped.find(tile) != prev_flipped.end()) {
            prev_flipped.erase(tile);
        } else {
            prev_flipped.emplace(tile);
        }
    }
    ...
    for (int day = 1; day <= 100; ++day) {
        ...
        for (const auto [x, y] : prev_flipped) {
            candidates.emplace(x, y);
            for (const auto [dx, dy] : moves) {
                candidates.emplace(x + dx, y + dy);
            }
        }
        unordered_set<pair<int, int>> next_flipped = prev_flipped;
        for (const auto [x, y] : candidates) {
            int adjacent = 0;
            for (const auto [dx, dy] : moves) {
                pair<int, int> tile = {x + dx, y + dy};
                adjacent += prev_flipped.find(tile) != prev_flipped.end();
            }
            pair<int, int> tile = {x, y};
            const bool black = prev_flipped.find(tile) != prev_flipped.end();
            if (black && (adjacent == 0 || adjacent > 2)) {
                next_flipped.erase(tile);
            } else if (!black && adjacent == 2) {
                next_flipped.emplace(tile);
            }
        }
        prev_flipped = next_flipped;
    }
    cout << prev_flipped.size() << '\n';
}
