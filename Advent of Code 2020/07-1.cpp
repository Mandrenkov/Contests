...
void solution() {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        const auto outsider = tokens[0] + " " + tokens[1];
        if (tokens[4] == "no") {
            // The current bag contains no other bags.
            continue;
        }
        for (int i = 4; i < tokens.size(); i += 4) {
            const auto insider = tokens[i + 1] + " " + tokens[i + 2];
            bags[insider].emplace_back(outsider);
        }
    }
    const string root = "shiny gold";
    unordered_set<string> visited;
    visited.emplace(root);
    const function<void(const string&)> explore = [&](const string& insider) {
        for (const auto& outsider : bags[insider]) {
            if (visited.find(outsider) == visited.end()) {
                visited.emplace(outsider);
                explore(outsider);
            }
        }
    };
    explore(root);
    int outsiders = visited.size() - 1;
    cout << outsiders << '\n';
}
